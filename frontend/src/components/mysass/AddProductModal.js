...
const AddProductModal = ({ product, onClose, onSave }) => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch('http://localhost:3011/api/product-categories?tenant=mysass');
        if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = product 
        ? `http://localhost:3011/api/products/${product._id}?tenant=mysass`
        : 'http://localhost:3011/api/products?tenant=mysass';
