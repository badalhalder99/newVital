...
const AddCertificateModal = ({ certificate, onClose, onCertificateSaved }) => {
  ...
  useEffect(() => {
    if (certificate) {
      ...
      if (certificate.logo) {
        setLogoPreview(certificate.logo);
      }
    }
  }, [certificate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = certificate
        ? `http://localhost:3011/api/certificates/${certificate._id}?tenant=mysass`
        : 'http://localhost:3011/api/certificates?tenant=mysass';
  ...
  return (
    <div className="mysass-modal-overlay" onClick={handleBackdropClick}>
      <div className="mysass-modal-content mysass-add-certificate-modal">
        <div className="mysass-modal-header">
          <h2>{certificate ? 'Edit Certificate' : 'Add Certificate'}</h2>
          <button className="mysass-modal-close" onClick={onClose}>
            <i className="fas fa-times"></i>
          </button>
        </div>

        <form onSubmit={handleSubmit} className="mysass-certificate-form">
          ...
          <div className="mysass-modal-actions">
            ...
            <button
              type="submit"
              className="mysass-btn-certificate-primary"
              disabled={loading}
            >
              {loading ? (
                <>
                  <i className="fas fa-spinner fa-spin"></i>
                  {certificate ? 'Updating...' : 'Adding...'}
                </>
              ) : (
                <>
