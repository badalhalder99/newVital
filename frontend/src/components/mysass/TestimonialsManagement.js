...
const TestimonialsManagement = () => {
  ...
  const fetchTestimonials = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3011/api/testimonials?tenant=mysass');
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await fetch(`http://localhost:3011/api/testimonials/${id}?tenant=mysass`, {
        method: 'DELETE'
      });
